...
architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_bram_writer is
  ...
  signal \address_prev[15]_i_1_n_0\ : STD_LOGIC;
...
\address[0]_i_3\: unisim.vcomponents.LUT1
    generic map(
      INIT => X"1"
    )
        port map (
...
\address_prev[15]_i_1\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"B"
    )
        port map (
      I0 => user_reset,
      I1 => reset_n,
      O => \address_prev[15]_i_1_n_0\
    );
\address_prev[15]_i_2\: unisim.vcomponents.LUT2
    generic map(
      INIT => X"8"
    )
        port map (
...
\address_prev_reg[15]\: unisim.vcomponents.FDRE
     port map (
      C => clk,
      CE => address_prev,
      D => address_reg(15),
