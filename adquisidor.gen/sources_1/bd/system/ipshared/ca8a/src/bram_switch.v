...
// El switch tiene mucho fanout y arruina el timing del diseño... voy a meter registros para solucionar eso...

(* DONT_TOUCH = "true" *) reg switch_1,switch_2,switch_3,switch_4,switch_5,switch_6,switch_7;

always @ (posedge bram_portc_clk)
begin
    switch_1 <= switch;
    switch_2 <= switch;
    switch_3 <= switch;
...
assign bram_portc_rst = switch_1 == 1 ? bram_porta_rst : bram_portb_rst;
assign bram_portc_addr = switch_2 == 1 ? bram_porta_addr : bram_portb_addr;
assign bram_portc_wrdata = switch_3 == 1 ? bram_porta_wrdata : bram_portb_wrdata;
assign bram_portc_we = switch_4 == 1 ? bram_porta_we: bram_portb_we;

assign bram_porta_rddata = switch_5 == 1 ? bram_portc_rddata : {(BRAM_DATA_WIDTH){1'b0}};
assign bram_portb_rddata = switch_6 == 0 ? bram_portc_rddata : {(BRAM_DATA_WIDTH){1'b0}};
...
BUFGMUX #(
   ...
   BUFGMUX_inst (
      ...
      .S(switch_7)    // 1-bit input: Clock select
   );
